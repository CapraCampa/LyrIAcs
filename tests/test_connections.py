...
class TestAppHealth(unittest.TestCase):
    ...
    def test_emotion_api(self):
        api_url_emotion = "https://lyriacs-api-emotion.onrender.com/predict_emotion_endpoint"
        data = {"text": "I am feeling great!"}
        try:
            response = requests.post(api_url_emotion, json=data)
    ...
    def test_genre_api(self):
        api_url_genre = "https://lyriacs-api-genre.onrender.com/predict_genre_endpoint"
        data = {"text": "Some song lyrics"}
        try:
            response = requests.post(api_url_genre, json=data)
